...
def count1():
	if 'user_id' in session:
		...
		session_user_firstname = session['user_firstname'] 
		session_account_type = session['account_type'] 
		count1=[]
		conn = connection()
		cursor = conn.cursor()
		cursor.execute("SELECT COUNT(CURRENT_DATE) AS date_count FROM IH_APPOINTMENT WHERE STATUS = 'Approved' AND appt_type='Clinic Service'")
		row = cursor.fetchone()
...
def updateschedule(clinic_sched_id):
	if('user_id' in session):
		...
		cursor = conn.cursor()
		if request.method == 'GET':
			cursor.execute("SELECT * FROM ih_clinic_sched WHERE clinic_sched_id = %s", (clinic_sched_id,))
			for row in cursor.fetchall():
...
def adminclinicinv():
	...
	for row in cursor.fetchall():
		adminclinicinv.append({"clinic_supp_id": row[0],"item_name": row[1], "qty": row[3]})
	conn.close()	
...
def updateclinicinv(clinic_supp_id):
	...
	if request.method == 'GET':
		...
		conn.close()
		return render_template("updateclinicinv.html", adminclinicinv = ucv[0])
	if request.method == 'POST':
		qty = int(request.form['qty'])
		cursor.execute("UPDATE ih_clinic_supply SET qty = %s  WHERE clinic_supp_id =(%s)",
		(qty,clinic_supp_id))
		conn.commit()
		conn.close()
		return redirect("/adminclinicinv")
...
def adminaptdental():
	...
	conn = connection()
	cursor = conn.cursor()
	cursor.execute("SELECT IH.APPT_ID,IH.APPT_TYPE, IH.DATE, IH.STATUS, U.FIRSTNAME, CSH.SCHEDULE_NAME FROM IH_APPOINTMENT IH INNER JOIN IH_CLINIC_SCHED CSH ON CSH.CLINIC_SCHED_ID = IH.CLINIC_SCHED_ID INNER JOIN USERS_USER U ON U.ID = IH.ID WHERE APPT_TYPE = 'Dental Service' AND STATUS='pending'")
	for row in cursor.fetchall():
...
def updatemedicinestaff(med_appt_id):
	...
	if request.method == 'POST':
		...
		qty = int(request.form['qty'])		
		cursor.execute("UPDATE ih_medicine_appointment SET (appt_type,date,medicine_id,qty) = (%s,%s,%s,%s)  WHERE med_appt_id =(%s)",
		(appt_type,date,medicine_id,qty,med_appt_id))
		conn.commit()
		conn.close()
		return redirect('/medicinestaff')
...
def managebookingscancel():
		...
		conn = connection()
		cursor = conn.cursor()
		cursor.execute("UPDATE ih_appointment SET status = 'Cancelled' WHERE appt_id = %s", (appt_id,))
		conn.close()	
...
def residenthaptdental():
	if('user_id' in session):
		...
		conn = connection()
		cursor = conn.cursor()
		cursor.execute("SELECT IH.APPT_ID, IH.APPT_TYPE, IH.DATE, IH.STATUS, U.FIRSTNAME, CSH.SCHEDULE_NAME FROM IH_APPOINTMENT IH INNER JOIN IH_CLINIC_SCHED CSH ON CSH.CLINIC_SCHED_ID = IH.CLINIC_SCHED_ID INNER JOIN USERS_USER U ON U.ID = IH.ID WHERE STATUS='Approved'AND APPT_TYPE='Dental Service' AND FIRSTNAME= %s", (user_name,))
		for row in cursor.fetchall():
...
def residenthaptmedicine():
	...
	conn = connection()	
	cursor = conn.cursor()
	cursor.execute("SELECT IHM.MED_APPT_ID,IHM.APPT_TYPE, IHM.DATE, IHM.STATUS, U.FIRSTNAME, M.MEDICINE_NAME, IHM.QTY,CSH.SCHEDULE_NAME, IHM.REMARKS FROM IH_MEDICINE_APPOINTMENT IHM INNER JOIN IH_MEDICINE M ON IHM.MEDICINE_ID = M.MEDICINE_ID INNER JOIN IH_CLINIC_SCHED CSH ON CSH.CLINIC_SCHED_ID = IHM.CLINIC_SCHED_ID INNER JOIN USERS_USER U ON U.ID = IHM.ID WHERE STATUS='Approved' AND FIRSTNAME= %s", (user_name,))
	for row in cursor.fetchall():
